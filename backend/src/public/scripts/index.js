...
function displayReports() {
    httpGet('/api/reports/all')
        .then(response => response.json())
        .then((response) => {
            var allReports = response.reports;
            var allReportsAnchor = document.getElementById('all-reports-anchor');
            allReportsAnchor.innerHTML = '';
            allReports.forEach((report) => {
                allReportsAnchor.innerHTML += getReportDisplayEle(report);
            })
        })
}
...
function getReportDisplayEle(report) {
    return `<div class="report-display-ele">
    
        <div class="normal-view">
            <div>ID: ${report.id}</div>
            <div> Start: ${report.startTime}</div>
...
document.addEventListener('click', function (event) {
    ...
    //     deleteUser(ele);
    if (ele.matches('.delete-report-btn')) {
        deleteReport(ele);
    }
}, false)
...
function deleteReport(ele) {
    var id = ele.getAttribute('data-report-id');
	httpDelete('/api/reports/delete/' + id)
        .then(() => {
            displayReports();
        })
}
